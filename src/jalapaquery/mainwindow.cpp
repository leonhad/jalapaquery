    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->actionAboutPlugins, SIGNAL(triggered()), this,
        SLOT(pluginDialog()));
    connect(ui->actionNewProject, SIGNAL(triggered()), this, SLOT(newFile()));

    // connect(m_mainWidget, SIGNAL(tabCloseRequested(int)), this,
    // SLOT(closeTab(int)));
    ...
    /*
      ...
      QPainterPath path;
      path.moveTo(item1->pos().x() + item1->boundingRect().width(), item1->pos().y()
      + item1->boundingRect().height() / 2); path.lineTo(item2->pos().x(),
      item2->y() + item2->boundingRect().height() / 2); // lazy

      QGraphicsPathItem *pathItem = scene->addPath(path, pen);
      pathItem->setFlags(QGraphicsItem::ItemIsMovable |
...
void MainWindow::loadPlugins() { loadModelPlugins(); }
